    ...
    <div className="pt-28 pb-16 min-h-screen text-brand-charcoal selection:bg-brand-gold/30">
      <div className="max-w-7xl mx-auto px-4 md:px-12 space-y-12">
        <div className="text-center max-w-2xl mx-auto space-y-4">
          <span className="text-xs uppercase tracking-widest font-extrabold text-brand-greenVibrant">Nurturing Learning Pathways</span>
          <h2 className="font-serif text-4xl sm:text-5xl font-bold text-brand-greenDeep">Our Pedagogy</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pt-8">
          {stages.map((stage, idx) => (
            <div key={stage.id} className="bg-white p-6 sm:p-8 rounded-3xl border border-brand-greenDeep/5 shadow-md flex flex-col justify-between hover:shadow-lg transition-shadow duration-300">
              <div className="space-y-4">
                <span className="text-4xl font-serif text-brand-gold font-bold">0{idx + 1}</span>
                <h3 className="font-serif text-lg font-bold text-brand-greenDeep">{stage.title}</h3>
