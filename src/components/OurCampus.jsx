    ...
    <div className="pt-24 bg-brand-bg min-h-screen text-brand-charcoal selection:bg-brand-gold/30">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-12 py-16 md:py-24">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-6 bg-brand-greenDeep/5 p-8 rounded-3xl border border-brand-greenDeep/10 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-brand-gold/10 rounded-full blur-3xl"></div>
            <p className="font-serif text-xl sm:text-2xl text-brand-greenDeep font-medium italic leading-relaxed relative z-10">
            ...
            <div className="mt-6 flex items-center gap-3">
              ...
              <span className="text-xs uppercase tracking-widest font-bold text-brand-muted">DLPS Legacy & Infrastructure</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-12 pb-24">
        <div className="text-center mb-12">
          <h2 className="font-serif text-3xl md:text-4xl font-bold text-brand-greenDeep tracking-tight">
            Visual Campus Portfolio
          </h2>
          <p className="text-xs text-brand-gold font-bold uppercase tracking-widest mt-2">
      ...
      <section className="bg-brand-greenDeep text-white py-20 relative overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-brand-greenVibrant/10 rounded-full blur-3xl pointer-events-none"></div>
        <div className="max-w-7xl mx-auto px-4 md:px-12 relative z-10">
          <div className="text-center max-w-2xl mx-auto mb-16">
            <span className="text-[10px] tracking-widest uppercase font-bold text-brand-gold">Nurturing Yet Protected</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {safetyFeatures.map((feat, idx) => {
              ...
              return (
                <div key={idx} className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-8 hover:border-white/20 transition-all duration-300">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-12 py-24">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-16">
          <div className="lg:col-span-5">
            <span className="text-[10px] tracking-widest uppercase font-bold text-brand-gold">Global Infrastructure</span>
            <h2 className="font-serif text-3xl sm:text-4xl font-bold text-brand-greenDeep mt-2 leading-tight">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {facilities.map((fac, idx) => {
            ...
            return (
              <div 
                key={idx} 
                className="bg-white border border-brand-greenDeep/5 hover:border-brand-greenDeep/15 hover:shadow-lg transition-all duration-300 rounded-3xl p-8 group"
              >
                <div className="w-12 h-12 rounded-2xl bg-brand-greenDeep/5 group-hover:bg-brand-greenDeep/10 flex items-center justify-center text-brand-greenDeep mb-6 transition-colors duration-300">
