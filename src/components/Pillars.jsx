...
export default function Pillars() {
  ...
  // Define solid background colors based on active branch
  const cardBgClass = activeBranch === 'dlf-sahibabad' ? 'bg-[#1b3518]' : 'bg-[#2c204d]'

  return (
    <section id="zero-waste" className={`py-16 sm:py-24 bg-transparent text-brand-bg relative overflow-hidden`}>
      {/* Floating shapes */}
      ...
      <div className="w-[96%] max-w-[1600px] mx-auto px-4 md:px-12 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 sm:gap-8 mb-12 sm:mb-16">
          <div className="max-w-2xl space-y-3 sm:space-y-4">
            <span className={`text-xs uppercase tracking-widest font-bold text-${theme.accent}`}>Defining Innovations</span>
          ...
          <div className="shrink-0 w-full sm:w-auto">
            <a href="#procedure" className={`group flex items-center justify-center gap-2 border border-${theme.accent}/30 hover:border-${theme.accent}/80 px-6 py-3 rounded-xl text-xs font-bold text-white uppercase tracking-wider transition-all duration-300 w-full sm:w-auto`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          {/* Card 1: Zero-Waste */}
          <div className={`${cardBgClass} border border-white/10 rounded-2xl p-6 sm:p-8 hover:-translate-y-2 transition-all duration-300 flex flex-col justify-between group min-h-[320px] relative overflow-hidden shadow-lg`}>
            <div>
              <div className={`w-10 h-10 bg-${theme.accent}/15 rounded-xl flex items-center justify-center text-${theme.accent} mb-5`}>
